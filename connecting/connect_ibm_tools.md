...
You can connect IBM command-line interfaces, applications, and tools to your {{site.data.keyword.dashdbshort_notm}} database. 
{: shortdesc}
...
It is advised that you update DataStage to the most recent version so that you can take advantage of external tables to load your data into {{site.data.keyword.dashdbshort_notm}}.
{: important}
...
You can connect IBM® InfoSphere® Data Replication to a {{site.data.keyword.dashdbshort_notm}} database. This capability applies to both SMP and MPP environments.  
{: shortdesc}
...
1. Choose one of the following approaches to make your connection:
   ...
   - To create a connection with SSL, complete the following steps:
     ...
     - Test the connection by using CLP by issuing the following command to connect to the {{site.data.keyword.dashdbshort_notm}} database:

       `db2 connect to <db_alias> user <user_id>`

       where `<user_id>` is your {{site.data.keyword.dashdbshort_notm}} user ID. You are prompted to enter your password.
                
     - Test the connection by using CLI by issuing the following command to connect to the {{site.data.keyword.dashdbshort_notm}} database:
   ...
   - To create a connection without SSL, complete the following steps:
     ...
     - Test the connection by using CLI by issuing the following command to connect to the {{site.data.keyword.dashdbshort_notm}} database:
       ...
       where `<alias>` is a DSN alias that you created by using the **db2cli writecfg** command, `<user_id>` is your {{site.data.keyword.dashdbshort_notm}} user ID, and `<password>` is your {{site.data.keyword.dashdbshort_notm}} password.
...
These instructions explain how to create a connection from IBM® Data Studio to a {{site.data.keyword.dashdbshort_notm}} database.
{: shortdesc}
